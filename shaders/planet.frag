...
vec3 in_scattering(vec3 viewer, vec3 frag_pos, vec3 light_dir, float radius, float atmos_height)
{
	...
	vec3 color = sum * (K.xyz*rayleigh(cc) + K.www*mie(c,cc));
...
void main()
{
	...
	vec3 scat = in_scattering(viewPos.xyz-planetPos.xyz, pp, lightDir.xyz, radius, atmoHeight);
	float angle_light = dot(norm_v, lightDir.xyz)*0.5+0.5;
	color = day*exp(-texture(atmo, vec2(angle_light,0)).g*(K.xyz+K.www))+scat+night;
#endif
